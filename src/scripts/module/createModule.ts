...
export class CreateModule extends OperationsModel {
  ...
  static constructModule(pathEntities: string, pathEntitiesExample: string) {
    ...
    this.createFolder(pathEntities, namePrincipalModule, nameSecondModule);
    this.createFileModule(pathEntities, pathEntitiesExample, namePrincipalModule, nameSecondModule, nameFileClass);
  }
  ...
  private static createFileModule(
    ...
    fs.readFile(
      ...
      (err: any, text: string) => {
        ...
        if (!fs.existsSync(`${pathEntities}/${namePrincipalModule}/${nameSecondModule}/${nameSecondModule}.module.ts`)) {
          fs.writeFileSync(`${pathEntities}/${namePrincipalModule}/${nameSecondModule}/${nameSecondModule}.module.ts`, data);
        }
      }
    );
  }
